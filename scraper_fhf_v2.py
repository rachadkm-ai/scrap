...
async def get_services(page, detail_url: str) -> list:
    ...
    Ouvre l'accordeon 'Services de l'etablissement'.
    Extrait chaque .row-item.
    """
    try:
    ...
    for el in row_els:
        ...
        services.append({"title": title, "service_h3": svc_h3, "mailto": mailto})
...
if __name__ == "__main__":
    ...
    if data:
        export_csv(data,   OUTPUT_DIR / "fhf_etablissements.csv")
        export_json(data,  OUTPUT_DIR / "fhf_etablissements.json")
        export_excel(data, OUTPUT_DIR / "fhf_etablissements.xlsx")
    else:
